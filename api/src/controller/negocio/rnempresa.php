...
class RNEmpresa{
	...
	public function alterar($empresa){
		try{
			...
			return array('sucess' => 'Cadastro Alterado com Sucesso!');
	...
	public function salvar(Empresa $empresa){
		try{
			...
			if (empty($empresa->getDsEndereco()))
				array_push($validacoes, 'Endereço precisa ser preenchido!');
			if (empty($empresa->getDsCep()))
				array_push($validacoes, 'O CEP precisa ser preenchido!');
